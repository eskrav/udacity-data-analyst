...
# a function to calculate summary statistics - adapted from https://stackoverflow.com/a/35954377
...
# a function to calculate summary statistics and generate a nicely formatted table from them
...
# a function to generate nice tables for summary statistics with two grouping variables
...
# collapses past due levels into one level, orders loan status levels, and calculates percentages of each loan status per Prosper rating
plot_data <- data %>% 
...
# function to plot categorical against continuous variables, producing a side-by-side boxplot and bar plot
...
# function to produce simple hex plot, to plot correlations between to quantitative variables - shows trends a bit better than simple scatterplot, given amount of data
# gam (general additive model) fits are in blue, lm (linear regression) fits are in organge.
...
# for simplicity, although many of the relationships here do not appear simply linear, I assume linear correlations - the main question for this preliminary exploration is usually only whether one variable generally causes the other to increase, or decrease
...
qplot(ProsperRating.num, ProsperScore, data = lender_data, geom=c("point", "jitter")) + 
  geom_smooth(formula = y~x, method=lm, color="orange")
```
...
cat_by_cont(filter(lender_data, !is.na(ProsperScore)), "factor(ProsperScore)", "PercentYield")
```

```{r lender profit by score table, cache=TRUE, fig.height=3.75}
table_stats(filter(lender_data, !is.na(ProsperScore)), "ProsperScore", "PercentYield")
```
...
cor.test(lender_data$EstimatedEffectiveYield, lender_data$PercentYield, method = "pearson")
```
</div>
...
cor.test(lender_data$EstimatedLoss, lender_data$PercentYield, method = "pearson")
```
</div>
...
cor.test(lender_data$EstimatedReturn, lender_data$PercentYield, method = "pearson")
```
</div>
...
demographics <- c("ProsperRating.num","ProsperScore","EmploymentStatusDuration","CreditScoreRangeLower","CreditScoreRangeUpper","FirstRecordedCreditLine","OpenRevolvingAccounts","InquiriesLast6Months","AmountDelinquent","DelinquenciesLast7Years","RevolvingCreditBalance","BankcardUtilization","DebtToIncomeRatio","TotalTrades","TradesNeverDelinquent.per")

# plots correlations among those borrower demographics and Prosper ratings
ggpairs(lender_data[demographics], lower = list(continuous = wrap("smooth", color = "blue")))
```
...
```{r loan status by occupation, cache=TRUE, warning=FALSE, message=FALSE, echo=FALSE}
plot_data <- lender_data %>% 
...
```{r lender profit by occupation, cache=TRUE, warning=FALSE, message=FALSE, echo=FALSE}
plot_data <- lender_data %>% 
  filter(!is.na(Occupation)) %>%
  group_by(Occupation) %>% 
...
plot_data <- lender_data %>% 
  filter(!is.na(EmploymentStatus)) %>%
  mutate(EmploymentStatus = factor(EmploymentStatus, levels =  c("Other","Not available","Self-employed","Not employed","Retired","Full-time","Employed","Part-time")))
...
cor.test(lender_data$EmploymentStatusDuration, lender_data$PercentYield, method = "pearson")
```
</div>
...
cat_by_cont(lender_data %>% 
  group_by(Completed) %>%
  filter(!is.na(IsBorrowerHomeowner)), "Completed", "as.numeric(IsBorrowerHomeowner)")
```
...
lender_data <- mutate(lender_data, CreditScore = (CreditScoreRangeLower+CreditScoreRangeUpper)/2)
...
cor.test(as.numeric(lender_data$CreditHistoryAge), lender_data$PercentYield, method="pearson")
```
</div>
...
cor.test(lender_data$OpenRevolvingAccounts, lender_data$PercentYield, method = "pearson")
```
</div>
...
cor.test(lender_data$InquiriesLast6Months, lender_data$PercentYield, method = "pearson")
```
</div>
...
simple_hex(filter(lender_data, AmountDelinquent < 30000), "AmountDelinquent", "PercentYield")
```
...
cor.test(lender_data$DelinquenciesLast7Years, lender_data$PercentYield, method = "pearson")
```
</div>
...
simple_hex(filter(lender_data, RevolvingCreditBalance < 500000), "RevolvingCreditBalance", "PercentYield")
```
...
cor.test(lender_data$BankcardUtilization, lender_data$PercentYield, method = "pearson")
```
</div>
...
cor.test(lender_data$DebtToIncomeRatio, lender_data$PercentYield, method = "pearson")
```
</div>
...
cor.test(lender_data$TotalTrades, lender_data$PercentYield, method = "pearson")
```
</div>
...
cor.test(lender_data$TradesNeverDelinquent.per, lender_data$PercentYield, method = "pearson")
```
</div>
...
prosper <- c("ProsperRating.num","ProsperScore","TotalProsperLoans","OnTimeProsperPayments","ProsperPrincipalOutstanding","Recommendations","InvestmentFromFriendsCount","InvestmentFromFriendsAmount")
...
cat_by_cont(replace_na(lender_data, list(TotalProsperLoans = 0)), "Completed", "TotalProsperLoans")
```
...
cat_by_cont(replace_na(lender_data, list(TotalProsperLoans = 0)), "factor(TotalProsperLoans)", "PercentYield")
```

```{r lender profit by prosper loans table, cache=TRUE, fig.height=3.25}
table_stats(replace_na(lender_data, list(TotalProsperLoans = 0)), "TotalProsperLoans", "PercentYield")
```
...
cor.test(lender_data$OnTimeProsperPayments, lender_data$PercentYield, method = "pearson")
```
</div>
...
cor.test(lender_data$ProsperPrincipalOutstanding, lender_data$PercentYield, method = "pearson")
```
</div>
...
cor.test(lender_data$Recommendations, lender_data$PercentYield, method = "pearson")
```
</div>
...
cor.test(lender_data$InvestmentFromFriendsCount, lender_data$PercentYield, method = "pearson")
```
</div>
...
cor.test(lender_data$InvestmentFromFriendsAmount, lender_data$PercentYield, method = "pearson")
```
</div>
...
borrower_profit <- c("ProsperRating.num","ProsperScore","EstimatedEffectiveYield","EstimatedLoss","EstimatedReturn","BorrowerAPR","BorrowerRate","LoanOriginalAmount","MonthlyLoanPayment","Term","PercentFunded")
...
```{r borrower terms by rating, cache=TRUE, warning=FALSE, message=FALSE, echo=FALSE}
...
complex_bar <- function(data, col) {
  ggplot(plot_data, aes_string(x = col, y="value")) +
...
```{r borrower terms by ratin table, cache=TRUE, fig.height=14, fig.width=9}
table_stats_mult(plot_data, "Measure", "Rating", "value")
```

#### `ProsperScore`

```{r borrower terms by sore, cache=TRUE, warning=FALSE, message=FALSE, echo=FALSE}
plot_data <- borrower_data %>%
...
```{r borrower terms by sore table, cache=TRUE, fig.height=19.5, fig.width=10}
...
complex_hex <- function(data,measure) {
  plot_data <- data %>%
  select(measure,BorrowerAPR,BorrowerRate,LoanOriginalAmount,MonthlyLoanPercent) %>%
...
# function to loop through relevant measures, printing out Pearson correlations with the variable of interest
...
```{r borrower terms by occupation, cache=TRUE, warning=FALSE, message=FALSE, echo=FALSE}
plot_data <- borrower_data %>% 
  filter(!is.na(Occupation)) %>%
  select(Occupation,BorrowerAPR,BorrowerRate,LoanOriginalAmount,MonthlyLoanPercent) %>%
  gather(Measure, value, -Occupation) %>%
  mutate(Occupation = reorder(Occupation, value))

complex_bar(plot_data, "Occupation")
```
...
```{r borrower terms by income range, cache=TRUE, warning=FALSE, message=FALSE, echo=FALSE}
plot_data <- borrower_data %>%
  filter(IncomeRange != "Not displayed") %>%
  select(IncomeRange,BorrowerAPR,BorrowerRate,LoanOriginalAmount,MonthlyLoanPercent) %>%
...
```{r borrower terms by income range 2, cache=TRUE, warning=FALSE, message=FALSE, echo=FALSE}
plot_data <- borrower_data %>%
...
```{r borrower terms by employment status, cache=TRUE, warning=FALSE, message=FALSE, echo=FALSE}
plot_data <- borrower_data %>%
  filter(!is.na(EmploymentStatus) & EmploymentStatus != "Not available") %>%
  select(EmploymentStatus,BorrowerAPR,BorrowerRate,LoanOriginalAmount,MonthlyLoanPercent) %>%
  gather(Measure, value, -EmploymentStatus) %>%
  mutate(EmploymentStatus = factor(EmploymentStatus, levels =  c("Other","Not available","Self-employed","Not employed","Retired","Employed","Part-time","Full-time")))
...
plot_data <- borrower_data %>%
  select(IsBorrowerHomeowner,BorrowerAPR,BorrowerRate,LoanOriginalAmount,MonthlyLoanPercent) %>%
...
plot_data <- borrower_data %>%
  select(IncomeVerifiable,BorrowerAPR,BorrowerRate,LoanOriginalAmount,MonthlyLoanPercent) %>%
...
```{r borrower terms by prosper loans, cache=TRUE, warning=FALSE, message=FALSE, echo=FALSE}
plot_data <- borrower_data %>%
  replace_na(list(TotalProsperLoans = 0)) %>%
  select(TotalProsperLoans,BorrowerAPR,BorrowerRate,LoanOriginalAmount,MonthlyLoanPercent) %>%
...
```{r borrower terms by loan quarter, cache=TRUE, warning=FALSE, message=FALSE, echo=FALSE}
plot_data <- borrower_data %>% 
  select(LoanOriginationQuarter,BorrowerAPR,BorrowerRate,LoanOriginalAmount,MonthlyLoanPercent) %>%
  gather(Measure, value, -LoanOriginationQuarter)

complex_bar(plot_data, "LoanOriginationQuarter")
```
...
plot_data <- data %>% 
  ...
  mutate(LoanStatus = ordered(LoanStatus, c("Defaulted","Chargedoff","PastDue","Current","FinalPaymentInProgress","Completed"))) %>%
  group_by(Rating, LoanStatus) %>% 
  tally %>% 
  mutate(percent = n/sum(n))
...
ggplot(plot_data, aes(x = Rating, y = percent)) +
  ...
  labs(title = "Loan Status by Rating", y = "Percent of Rating", x = "Loan Rating") +
...
plot_data_sum_est <- lender_data %>%
    filter(!is.na(ProsperScore)) %>%
    group_by(ProsperScore) %>%
    summarise(mean = round(mean(EstimatedReturn, na.rm = TRUE),3),
            median = round(median(EstimatedReturn, na.rm = TRUE),3),
            sd = round(sd(EstimatedReturn, na.rm = TRUE),3),
            n = n()) %>%
    mutate(se = round(sd / sqrt(n),3),
         lower.ci = round(mean - qt(1 - (0.05 / 2), n - 1) * se,3),
         upper.ci = round(mean + qt(1 - (0.05 / 2), n - 1) * se,3))
...
plot_data_sum_act <- lender_data %>%
    filter(!is.na(ProsperScore)) %>%
    group_by(ProsperScore) %>%
    summarise(mean = round(mean(PercentYield, na.rm = TRUE),3),
            median = round(median(PercentYield, na.rm = TRUE),3),
            sd = round(sd(PercentYield, na.rm = TRUE),3),
            n = n()) %>%
    mutate(se = round(sd / sqrt(n),3),
         lower.ci = round(mean - qt(1 - (0.05 / 2), n - 1) * se,3),
         upper.ci = round(mean + qt(1 - (0.05 / 2), n - 1) * se,3))
...
bar1 <- ggplot(plot_data_sum_est, aes(x = factor(ProsperScore), y=mean)) + 
  geom_bar(stat="identity") + 
  geom_errorbar(aes(ymin=lower.ci, ymax=upper.ci), size=0.5, width=.25,position=position_dodge(.9)) + 
  scale_y_continuous(labels = percent) + 
  labs(x = "Prosper Score", y = "Estimated Return")
...
grid.arrange(top = textGrob("Estimated and Actual Yield by Prosper Score", gp = gpar(fontsize=18)), box1, bar1, box2, bar2 ,ncol=2)
```
...
plot_data_sum_comp <- lender_data %>%
    group_by(LoanOriginationQuarter) %>%
    summarise(mean = round(mean(Completed.num, na.rm = TRUE),3),
            median = round(median(Completed.num, na.rm = TRUE),3),
            sd = round(sd(Completed.num, na.rm = TRUE),3),
            n = n()) %>%
    mutate(se = round(sd / sqrt(n),3),
         lower.ci = round(mean - qt(1 - (0.05 / 2), n - 1) * se,3),
         upper.ci = round(mean + qt(1 - (0.05 / 2), n - 1) * se,3))
...
plot_data_sum_yield <- lender_data %>%
    group_by(LoanOriginationQuarter) %>%
    summarise(mean = round(mean(PercentYield, na.rm = TRUE),3),
            median = round(median(PercentYield, na.rm = TRUE),3),
            sd = round(sd(PercentYield, na.rm = TRUE),3),
...
quarter1 <- ggplot(plot_data_sum_comp, aes(x = factor(LoanOriginationQuarter), y=mean)) +
  geom_bar(stat="identity") + 
  geom_errorbar(aes(ymin=lower.ci, ymax=upper.ci), size=0.5, width=.25,position=position_dodge(.9)) + 
  scale_y_continuous(labels = percent) + 
  coord_flip() + 
...
quarter2 <- ggplot(plot_data_sum_yield, aes(x = factor(LoanOriginationQuarter), y=mean)) +
  geom_bar(stat="identity") + 
  geom_errorbar(aes(ymin=lower.ci, ymax=upper.ci), size=0.5, width=.25,position=position_dodge(.9)) + 
  scale_y_continuous(labels = percent) + 
  coord_flip() + 
...
grid.arrange(top = textGrob("Loan Completion and Profit by Loan Origination Quarter", gp = gpar(fontsize=18)), quarter1, quarter2, ncol=2)
```
